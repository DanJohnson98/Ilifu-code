...
run_parameters = load_object("run_parameters")

num_per_run = run_parameters["num_per_run"]
n_burn = run_parameters["n_burn"]
n_run = run_parameters["n_run"]
threadCount = run_parameters["threadCount"]
...
fitting_kwargs_list = [['MCMC',
                        {'n_burn': n_burn, 'n_run': n_run,
                         'walkerRatio': walker_ratio, 'sigma_scale': 1., 'threadCount': threadCount}]]
...
if nn*num_per_run >= n_models_use:
    run = False
else:
    min_model = nn*num_per_run
    if n_models_use < (nn+1)*num_per_run:
        max_model = n_models_use
    else:
        max_model = (nn+1)*num_per_run

if run:
    for i in range(min_model,max_model):
